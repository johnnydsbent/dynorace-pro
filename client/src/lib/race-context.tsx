...
export function RaceProvider({ children }: { children: React.ReactNode }) {
  ...
  const createRace = useCallback((raceType: RaceType, leftCar: Car, rightCar: Car, rollSpeed?: 20 | 40 | 60) => {
    const isLeftObd = leftCar.id === "obd-vehicle";
    const isRightObd = rightCar.id === "obd-vehicle";
    ...
    const race: Race = {
      ...
      status: "setup",
      participants: [
        { carId: leftCar.id, lane: "left", driverName: isLeftObd ? "Your Vehicle" : "Driver 1", isObd: isLeftObd, car: leftCar },
        { carId: rightCar.id, lane: "right", driverName: isRightObd ? "Your Vehicle" : "Driver 2", isObd: isRightObd, car: rightCar },
      ],
      results: [],
    ...
    setCurrentRace(prev => {
      ...
      setDynoHealth(health => health.map((d, i) => {
        ...
        const log: PassLog = {
          ...
          raceId: completed.id,
          dynoId: d.dynoId,
          carName: car?.name || (participant?.isObd ? "Your Vehicle (OBD)" : "Unknown"),
  ...
  const refreshCars = useCallback(async () => {
    try {
      ...
      setDynoMachines(prev => {
        const isObd = prev[0]?.carId === "obd-vehicle";
        return [
          { id: "dyno1", name: "Dynojet 1", status: "connected", carId: isObd ? "obd-vehicle" : (loadedCars[0]?.id || null) },
          { id: "dyno2", name: "Dynojet 2", status: "connected", carId: loadedCars[1]?.id || null },
        ];
      });
    } catch (error) {
