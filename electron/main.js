...
electron_1.ipcMain.handle("obd:connect", async (_event, portPath) => {
    try {
        ...
        parser = serialPort.pipe(new serialport_2.ReadlineParser({ delimiter: "\r" }));
        parser.on("data", (data) => {
...
electron_1.ipcMain.handle("obd:send-command", async (_event, command) => {
    try {
        if (!serialPort?.isOpen) {
...
electron_1.ipcMain.handle("obd:start-live-data", async () => {
    if (!serialPort?.isOpen) {
...
electron_1.ipcMain.handle("obd:stop-live-data", async () => {
    stopPollingOBDData();
    return { success: true };
});
...
async function pollOBDData() {
    if (!serialPort?.isOpen || !mainWindow)
        return;
    try {
